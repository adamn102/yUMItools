    ...
    * generate quality metrics
    ...
    def library_pipeline(self, cutoff=5, subset=False, sub_value=1000):
        ...
        for barcode in barcodes:
            ...
            umi_df = self.consensus_df(corrected_barcode_dict, min_coverage=cutoff, subset=subset, sub_value=sub_value)
    ...
    def consensus_df(self, barcode_dict, min_coverage=5,subset=False, sub_value=1000):
        ...
        for umi in barcode_dict.keys():
            ...
            if len(barcode_dict[umi]) >= min_coverage:
                if count == 0:
                    df = self.consensus_caller(umi, barcode_dict, min_coverage, subset, sub_value)
                    if type(df) != int:
                        count += 1
                else:
                    df2 = self.consensus_caller(umi, barcode_dict, min_coverage, subset, sub_value)
                    if type(df2) != int:
                        df = df.append(df2)
